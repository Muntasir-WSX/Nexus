...
export default function RightSidebar({ selectedContact, activeUser }) {
	const containerVariants = {
		...
		show: {
			opacity: 1,
			x: 0,
			transition: { duration: 0.28, ease: "easeOut", when: "beforeChildren", staggerChildren: 0.07 },
		},
	};
	...
	return (
		<motion.div
			...
			initial="hidden"
			animate="show"
			className="flex h-full w-full flex-col gap-4 xl:w-[320px] xl:max-w-[320px]"
		>
			...
			<motion.section variants={itemVariants} className="rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm">
				...
				<div className="rounded-2xl bg-white p-3 shadow-sm">
					...
					<motion.div className="space-y-2" variants={{ show: { transition: { staggerChildren: 0.05 } } }}>
						{notes.map((note, index) => (
							<motion.div key={note} variants={itemVariants} className="flex items-start gap-2">
								<span
			...
			<motion.section variants={itemVariants} className="flex min-h-0 flex-1 flex-col rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm">
				<div className="mb-3 flex items-center justify-between">
				...
				<div className="custom-scrollbar flex-1 space-y-3 overflow-y-auto pr-1">
					<motion.div variants={itemVariants} className="w-[82%] rounded-2xl rounded-bl-sm bg-[#F7F7EE] p-3 text-[12px] text-gray-700 shadow-sm">
						Hey {activeUser?.name?.split(" ")[0] || "there"}, I need to check something about this deal.
					</motion.div>

					<motion.div variants={itemVariants} className="ml-auto flex w-[82%] justify-end gap-2">
					...
					<motion.div variants={itemVariants} className="flex items-center justify-end gap-2 pr-1">
				...
				<motion.div variants={itemVariants} className="mt-3 flex items-center gap-2 rounded-2xl bg-white px-3 py-2 shadow-sm">
					<input
						type="text"
